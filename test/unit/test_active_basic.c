...
ACTIVERT_EVENT_POOL_DEFINE( basic_pool, basic_test_event_t, BASIC_POOL_SIZE,
...
ACTIVERT_ACTIVE_DEFINE_SIMPLE( basic_ao, basic_dispatch, BASIC_AO_PRIORITY,
                                BASIC_AO_STACK_BYTES, BASIC_AO_QUEUE_DEPTH );
 ...
 *--------------------------------------------------------------------------*/
static BaseType_t post_and_wait( uint32_t sig, uint32_t payload,
                                 TickType_t timeout_ticks )
{
    basic_test_event_t *evt =
        ( basic_test_event_t * ) activert_event_pool_alloc( basic_pool );
    if ( evt == NULL )
        return pdFAIL;
...
void setUp( void )
{
    ...
    ACTIVERT_EVENT_POOL_INIT( basic_pool, basic_test_event_t, BASIC_POOL_SIZE,
                              ACTIVERT_POOL_OVERFLOW_DROP );
