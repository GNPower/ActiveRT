    ...
    while (1) {
        ...
        *************************************************************/
        if (me->notification.handler) {
            ...
            if (xTaskNotifyWait(0, 0xFFFFFFFF, &notify_bits, 0) == pdPASS) {
        ...
        *************************************************************/
        ...
        if (me->queue_set) {
            ...
            if (me->notification.semaphore && active_queue == me->notification.semaphore) {
                ...
                // Call notification handler
                if (me->notification.handler && notify_value != 0) {
...
static activert_active_t* activert_active_create_static_internal(
    ...
    activert_event_t*** queue_storages,
...
)
{
    ...
    ACTIVERT_ASSERT(num_queues == 1 || queue_set_cb != NULL);
    ...
    if (num_queues > 1 || me->notification.semaphore != NULL) {
        ...
        if (me->notification.semaphore != NULL) {
            queue_set_size += 1;  // Binary semaphore counts as 1 slot
        }
...
void activert_active_print_stats(activert_active_t* me) {
    ...
    if (me->stats.events_processed > 0) {
        ...
        printf("  Max time:     %u ticks (sig %u)\n", 
               (uint32_t)me->stats.max_processing_time, me->stats.slowest_signal);
    }
    #endif
    ...
    for (uint8_t i = 0; i < me->queue_count; i++) {
        ...
        printf("  Peak:         %u / %zu (%u%%)\n",
               ...
               me->queues[i].queue_length,
               (uint32_t)((me->queues[i].stats.peak_depth * 100) / me->queues[i].queue_length));
    }
