...
bool activert_queue_is_full(
    ...
    return (uxQueueSpacesAvailable(me->queues[queue_index].handle) == 0);
}
...
bool activert_queue_is_empty(
    ...
    return (uxQueueMessagesWaiting(me->queues[queue_index].handle) == 0);
}
...
uint32_t activert_queue_flush(
    ...
    printf("activert_queue_flush: Flushed %u events from queue %u in task '%s'\n",
           flushed, queue_index, me->name ? me->name : "unnamed");
    #endif
    #endif
...
void activert_queue_print_stats(
    ...
    if (queue->signal_count == 0) {
        ...
        printf("  Signals:      %u - %u (%u signals)\n",
               queue->signal_base,
               queue->signal_base + queue->signal_count - 1,
               queue->signal_count);
    }
    ...
    printf("  Current:      %u / %zu (%u%%)\n",
           current_depth, queue->queue_length,
           (uint32_t)((current_depth * 100) / queue->queue_length));
    ...
    if (queue->stats.posts_attempted > 0) {
        float success_rate = (float)queue->stats.posts_succeeded * 100.0f /
                            (float)queue->stats.posts_attempted;
    ...
    printf("  Peak:         %u / %zu (%u%%)\n",
           ...
           queue->queue_length,
           (uint32_t)((queue->stats.peak_depth * 100) / queue->queue_length));
}
...
uint8_t activert_queue_get_utilization(
    ...
    UBaseType_t depth = uxQueueMessagesWaiting(me->queues[queue_index].handle);
    size_t length = me->queues[queue_index].queue_length;
    
    return (uint8_t)((depth * 100) / length);
}
...
uint8_t activert_queue_get_peak_utilization(
    ...
    return (uint8_t)((peak * 100) / length);
}
