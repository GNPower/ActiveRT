...
void activert_cli_cmd_summary(const char* args) {
    ...
    ACTIVERT_CLI_PRINTF("Events Processed: %u", (unsigned int)activert_stats_get_total_events_processed());
    ACTIVERT_CLI_PRINTF("Events Dropped:   %u", (unsigned int)activert_stats_get_total_events_dropped());
    ACTIVERT_CLI_PRINTF("Notifications:    %u", (unsigned int)activert_stats_get_total_notifications());
    ACTIVERT_CLI_PRINTF("Pool Allocs:      %u", (unsigned int)activert_stats_get_total_pool_allocs());
    ACTIVERT_CLI_PRINTF("Pool Failures:    %u", (unsigned int)activert_stats_get_total_pool_failures());
    ...
    switch (health.status) {
        ...
        case ACTIVERT_HEALTH_CRITICAL:
            ACTIVERT_CLI_PRINTF("Health: CRITICAL (%u warnings, %u critical)",
                               (unsigned int)health.warnings, (unsigned int)health.criticals);
            break;
    }
}
...
void activert_cli_cmd_pool(const char* args) {
    ...
    if (pool->stats.allocs_attempted > 0) {
        uint32_t rate = (pool->stats.allocs_succeeded * 100) / pool->stats.allocs_attempted;
...
const char* activert_cli_health_status_str(activert_health_status_t status) {
    switch (status) {
        case ACTIVERT_HEALTH_OK:       return "OK";
        case ACTIVERT_HEALTH_WARNING:  return "WARNING";
        case ACTIVERT_HEALTH_CRITICAL: return "CRITICAL";
...
) 
{
    if (bytes < 1024) {
        ...
        snprintf(buffer, buffer_len, "%u KB", (unsigned int)(bytes / 1024));
    } else {
        snprintf(buffer, buffer_len, "%u MB", (unsigned int)(bytes / (1024 * 1024)));
    }
    return buffer;
}
...
) 
{
    if (max == 0) {
        ...
        uint32_t percent = (value * 100) / max;
